...
export default function App() {
  ...
  const updateCustomer = async (customer) => {
    return new Promise((resolve, reject) => {
      db.transaction(
        tx => {
          tx.executeSql(`update customers set name=? where uid=${customer.uid}`, [customer.name]);
        },
        reject,
        resolve
      );
    ...
    const deleteCustomerSQLite = async (uid) => {
      return new Promise((resolve, reject) => {
        db.transaction(
          tx => {
            tx.executeSql(`delete from customers where uid= ?`, [uid]);
          },
          reject,
          resolve
        );
  ...
  const deleteCustomer = async (customer) => {
    ...
    setCustomers(newCustomers); 
    deleteCustomerSQLite(customer.uid);
    // 1. Set the new local customer state
        ...
        <View style={styles.container}>
          ...
          <View>
            ...
            {customers.map((customer) => (
              <View style={styles.customer} key={customer.uid}>
